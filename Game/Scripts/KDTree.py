...
class KDTree:
	...
	def setDimensions(self, element):
		...
		element. Override this if the elements are more than just points
		(vectors).'''
...
class BlenderObjectXYTree(KDTree):
	...
	def bakeClusters(self, n = None):
		...
		of each node, will be saved in the objects as properties.
		'''
		if n == None:
			n = self.root
		if n == None:
...
class GameObjectXYTree(KDTree):
	...
	def elemCmp(self, e1, e2, axis):
		...
		specified axis. axis will be 0 (x) or 1 (y), so it needs no
		modification.'''
		return lambda e1, e2: e1.worldPosition[axis] - e2.worldPosition[axis]
	
	def getValue(self, e, axis):
		'''Get the position of an element on a given axis.'''
		return e.worldPosition[axis]
